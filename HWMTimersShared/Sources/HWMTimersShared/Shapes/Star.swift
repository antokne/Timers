...
public struct Star: Shape {
	...
	public func path(in rect: CGRect) -> Path {
		...
		let transform = CGAffineTransform(translationX: rect.origin.x + center.x, y: rect.origin.y + center.y + unusedSpace)
	...
	public var body: some View {
		ZStack {
			Canvas { context, size in
				...
				let shape = Star(corners: 7, smoothness: 0.5).path(in: CGRect(x: 50, y: 100, width: 50, height: 50))
								
				context.fill(shape, with: .color(.red))
				
				let star = Star(corners: 4, smoothness: 0.5).path(in: CGRect(x: 100, y: 200, width: 10, height: 10))
				context.fill(star, with: .color(.blue))
			}
		}
	}
}
